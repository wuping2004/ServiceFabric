...
namespace PingActor1
{
    ...
    internal class PingActor1 : Actor, IPingActor1, IRemindable
    {
        ...
        public async Task<string> Add(int first, int second)
        {
            
            for (int i = 0; i < 100; i++)
            {
                var state = new double[100000];
                Random random = new Random();
                state[random.Next(maxValue: 99)] = random.NextDouble();
                await this.StateManager.TryAddStateAsync($"State{i}", state);
            }
